...
exports.getAllUsers = async (req, res) => {
  try {
    const [users] = await db.query(
      "SELECT id, username, email, role, profile_image_url, is_verified, created_at, updated_at FROM users ORDER BY created_at DESC"
    );
...
exports.getUserById = async (req, res) => {
  try {
    const { id } = req.params;
    const [users] = await db.query(
      "SELECT id, username, email, role, profile_image_url, is_verified, created_at, updated_at FROM users WHERE id = ?",
      [id]
    );
